<template>
    <ExpandLayout class="relative h-full">
        
        <!-- Open the starts panel -->
        <template v-slot:actions="{ isExpanded, setIsExpanded}">
            <NavigationToggleButton @set-is-expanded="setIsExpanded" :is-expanded="isExpanded"></NavigationToggleButton>
        </template>

        <!-- the Starts Panel -->
        <template v-slot:expanded-content="{ isExpanded, setIsExpanded }">

            <div
                class="start-panel full-screen fixed top-0 left-0 grid justify-end items-end pb-14 px-2 transition-all"
                :class="[isExpanded ? 'active' : 'inactive']"
            >

                <fluent-card class="relative backdrop-blur-lg bg-white/80 dark:bg-black/80  w-full max-w-2xl overflow-auto max-h-[24rem]">

                    <!-- Content in here -->
                    <FlexLayout class="flex-col p-4 gap-8 justify-between h-full">
                        <GridLayout class="grid-cols-3 gap-2">
                            <IconButton has-hover has-active>
                                <div class="px-8 py-4 grid place-content-center">
                                    <Icon>wifi</Icon>
                                </div>
                            </IconButton>
                            <IconButton has-hover has-active>
                                <div class="px-8 py-4 grid place-content-center">
                                    <Icon>bluetooth</Icon>
                                </div>
                            </IconButton>
                            <IconButton has-hover has-active>
                                <div class="px-8 py-4 grid place-content-center">
                                    <Icon>air</Icon>
                                </div>
                            </IconButton>
                            <NightLightNavigationButton></NightLightNavigationButton>
                        </GridLayout>
                        
                        <NavigationSystemControl></NavigationSystemControl>
                    </FlexLayout>
                </fluent-card>

                <!-- z-index set need near relative or fixed -->
                <div
                    @click="setIsExpanded(false)"
                    class="fixed left-0 bottom-0 w-screen h-screen -z-[1]"
                    v-show="isExpanded"
                ></div>
            </div>
        </template>
    </ExpandLayout>
</template>

<script setup>
import NavigationSystemControl from './NavigationSystemControl.vue';
import NightLightNavigationButton from './NightLightNavigationButton.vue';
import NavigationToggleButton from './NavigationToggleButton.vue';

</script>

<style lang="scss" scoped>

</style>